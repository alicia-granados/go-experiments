...
func Test_app_enableCORS(t *testing.T) {
	...
	for _, e := range tests {
		...
		if e.expectedHeader && rr.Header().Get("Acces-Control-Allow-Credentials") == "" {
...
func Test_app_authRequired(t *testing.T) {
	...
	var tests = []struct {
		...
		{name: "invalid token", token: fmt.Sprintf("Bearer %s", expiredToken), expectAuthorized: false, setHeader: false},
	}
	...
	for _, e := range tests {
		...
		if !e.expectAuthorized && rr.Code != http.StatusUnauthorized {
			t.Errorf("%s: did not code 401, and should have", e.name)
		}
	}
}
